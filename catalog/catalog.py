...
def scrape_raw():
                    ...
                    if not reqs_tab:
                        ...
                        if container is None:
                            ...
                            for href in hrefs:
                                ...
                                result = get_reqs(thread_dom, program_name in no_reqs_tabs)
                                if result:
                                    degree_dict['threads'][result[1]] = result[0]
                    else:
                        result = get_reqs(degree_dom)
                        if result:
    ...
    log.info("{} of {} degrees had no concentrations.".format(str(num_degrees_no_concentrations), str(num_degrees)))
    log.info("Have you noticed that EVERY degree is a science degree, according to our catalog?")
    return catalog_dict


def scrape_firebase(): # actual main
    config = {
        ...
        "serviceAccount": "buzzplan-d333f-firebase-adminsdk-jhebg-c6fbec263e.json"
    }

    # Initialize the Firebase database reference
    firebase = pyrebase.initialize_app(config)
    db = firebase.database()
    catalog_dict = scrape_raw()
    db.child('catalog').push(json.dumps(catalog_dict))
